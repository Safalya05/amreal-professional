...
function Navbar() {
  ...
  return (
    <header className={cx("navbar", scrolled && "navbar-scrolled")}>
      <Link className="logo" to="/" onClick={() => setOpen(false)}>AMREAL<span>®</span></Link>
      ...
      <AnimatePresence>
        {open && (
          <motion.div className="mobile-menu" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
            <div className="mobile-menu-top"><Link className="logo" to="/" onClick={() => setOpen(false)}>AMREAL<span>®</span></Link><button onClick={() => setOpen(false)}><X/></button></div>
...
function Home() {
  ...
  return <main>
    ...
    <section className="social section blush">
      ...
      <div className="social-grid">{["1515377905703-c4788e51af15","1522337360788-8b13dee7a37e","1519735777090-ec97162dc266","1608248597279-f99d160bfcbc","1526047932273-341f2a7631f9","1516975080664-ed2fc6a32937"].map((id,i)=><a href="#" onClick={e=>e.preventDefault()} key={id} className="social-card"><img src={`https://images.unsplash.com/photo-${id}?auto=format&fit=crop&w=700&q=80`} alt="AMREAL social demo"/><span><Instagram size={18}/> View Post</span></a>)}</div>
    </section>
...
function Admin() {
  ...
  return <><main className="admin"><aside className="admin-side"><Link className="logo" to="/">AMREAL<span>®</span></Link>{["Dashboard","Products","Enquiries","Customers","Appointments","Feedback","Analytics"].map(x=><a href="#" onClick={e=>e.preventDefault()} key={x}>{x}</a>)}</aside><section className="admin-main"><div className="admin-top"><div><span className="eyebrow">ADMIN CRM</span><h1>OVERVIEW</h1></div><Link className="button button-small" to="/">View Website</Link></div><div className="stats">{stats.map(([x,y])=><div className="stat" key={x}><span>{x}</span><strong>{y}</strong></div>)}</div><div className="admin-table"><div className="table-title"><h2>Recent Enquiries</h2><span>Demo data</span></div><table><thead><tr><th>Name</th><th>Business</th><th>Product</th><th>Status</th></tr></thead><tbody>{products.slice(0,6).map((p,i)=><tr key={p.id}><td>Customer {i+1}</td><td>Professional Business</td><td>{p.name}</td><td><b className="status">{["NEW","CONTACTED","QUALIFIED"][i%3]}</b></td></tr>)}</tbody></table></div></section></main></>
}
...
function Footer() {
  return <footer className="footer"><div className="footer-main"><div><Link className="logo" to="/">AMREAL<span>®</span></Link><p>SCIENCE FOR BEAUTIFUL HAIR.</p></div><div><span className="footer-title">Explore</span><Link to="/products">Products</Link><Link to="/about">About</Link><Link to="/feedback">Feedback</Link></div><div><span className="footer-title">Connect</span><Link to="/contact">Business Enquiry</Link><a href="mailto:hello@amrealprofessional.com">Email</a><a href="tel:+910000000000">Phone</a></div><div><span className="footer-title">Social</span><a href="#" onClick={e=>e.preventDefault()}><Instagram size={17}/> Instagram</a></div></div><div className="footer-bottom"><span>© {new Date().getFullYear()} AMREAL Professional. Demo website.</span><span>No cart • No checkout • Enquiry-led experience</span></div></footer>
}
